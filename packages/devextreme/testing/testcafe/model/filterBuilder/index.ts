...
export default class FilterBuilder extends Widget {
  ...
  getGroupByLevel(level: number): Selector {
    return this.element.find(`.${CLASS.group}`).nth(level - 1);
  }

  getGroupItem(level?: number): Selector {
    if (level) {
  ...
  getOperationButton(level?: number): Selector {
    if (level) {
  ...
  getAddButton(level?: number): Selector {
    if (level) {
  ...
  getRemoveButton(index?: number): Selector {
    if (index) {
  ...
  getItem(item: string, index?: number): Selector {
    let className;
    switch (item) {
      case 'operation':
